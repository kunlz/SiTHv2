...
% ----------------------- %
% ======================================================================= %
function [wa, zgw, Tr, Es, uex, theta_cs] = sw_balance(IWS, pEc, pEs, Ta, Topt, s_VOD, ...
    ...
    elseif zgw > 0 && zgw <= zm(1)
        [wa, zgw, Tr, Es, uex, theta_cs] = swb_case1(wa, IWS, pEc, pEs, s_tem, s_VOD, ...
            soilpar, pftpar, wet, zm, zgw);

        % Case 2: groundwater table in layer 2
    elseif zgw > zm(1) && zgw <= zm(1) + zm(2)
        [wa, zgw, Tr, Es, uex, theta_cs] = swb_case2(wa, IWS, pEc, pEs, s_tem, s_VOD, ...
            soilpar, pftpar, wet, zm, zgw);

        % Case 3: groundwater table in layer 3
    elseif zgw > zm(1) + zm(2) && zgw < zm(1) + zm(2) + zm(3)
        [wa, zgw, Tr, Es, uex, theta_cs] = swb_case3(wa, IWS, pEc, pEs, s_tem, s_VOD, ...
            soilpar, pftpar, wet, zm, zgw);

        % Case 4: groundwater table below layer 3
    else
        [wa, zgw, Tr, Es, uex, theta_cs] = swb_case4(wa, IWS, pEc, pEs, s_tem, s_VOD, ...
            soilpar, pftpar, wet, zm, zgw);
...
% Case 0 -- groundwater overflow %
% ------------------------------ %
function [wa, zgw, Tr, Es, uex] = swb_case0(wa, IWS, pEc, pEs, s_tem, s_vpd, ...
    soilpar, pftpar, wet, zm, zgw)
    % function input:
    % ----------
    ...
    Tr1 = s_vpd * s_tem * Tr_p1;
    Tr2 = s_vpd * s_tem * Tr_p2;
    Tr3 = s_vpd * s_tem * Tr_p3;
    Tr = Tr1 + Tr2 + Tr3;
...
% Case 1 -- groundwater table in layer 1  %
% --------------------------------------- %
function [wa, zgw, Tr, Es, uex, theta_cs] = swb_case1(wa, IWS, pEc, pEs, s_tem, s_vpd, ...
    soilpar, pftpar, wet, zm, zgw)
    % function input:
    % ----------
    % wa      -- soil water content, 3 layers
    ...
    * theta_sat);
    ...
    [f_sm1, f_sm_s1, theta_cs] = swc_stress(wa1, soilpar, pEc, pftpar);

    % actual transpiration
    Tr1_u = f_sm1 * s_vpd * s_tem * Tr_p1_u;
    Tr1_u = max(Tr1_u, 0); % reject negtive value
    Tr1_u = min(Tr1_u, d1 * (wa1_unsat - wwp)); % less than maximum avaliable water
    Tr1_g = s_vpd * s_tem * Tr_p1_g;
    Tr1 = Tr1_u + Tr1_g;
    Tr2 = s_vpd * s_tem * Tr_p2;
    Tr3 = s_vpd * s_tem * Tr_p3;
    ...
    Es_u = Es * (d1 * wa1_unsat) / (d1 * wa1_unsat + (zm(1) - d1) ...
        * theta_sat);
    Es_u = min(Es_u, d1 * (wa1_unsat - wwp));
    Es_u = max(Es_u, 0);
...
% Case 2 -- groundwater table in layer 2  %
% --------------------------------------- %
function [wa, zgw, Tr, Es, uex, theta_cs] = swb_case2(wa, IWS, pEc, pEs, s_tem, s_vpd, ...
    soilpar, pftpar, wet, zm, zgw)
    % function input:
    % ----------
    % wa      -- soil water content, 3 layers
    ...
    [f_sm2, ~, theta_cs] = swc_stress(wa2_unsat, soilpar, pEc, pftpar);

    % actual transpiration
    Tr1 = f_sm1 * s_vpd * s_tem * Tr_p1;
    Tr2_u = f_sm2 * s_vpd * s_tem * Tr_p2_u;
    Tr2_g = s_vpd * s_tem * Tr_p2_g;
    Tr2 = Tr2_u + Tr2_g;
    Tr3 = s_vpd * s_tem * Tr_p3;
    Tr = Tr1 + Tr2 + Tr3;
    ...
    if wa1 - wwp > 0 && Es + Tr1 > d1 * (wa1 - wwp) % wilting point
        Tr1 = d1 * (wa1 - wwp) * Tr1 / (Tr1 + Es);
        Es = d1 * (wa1 - wwp) - Tr1;
    end
    ...
    wa2_unsat = max(wa2_unsat, wwp); % > wwp
    wa2_unsat = min(wa2_unsat, theta_sat); % < theta_s
...
% Case 3 -- groundwater table in layer 3  %
% --------------------------------------- %
function [wa, zgw, Tr, Es, uex, theta_cs] = swb_case3(wa, IWS, pEc, pEs, s_tem, s_vpd, ...
    soilpar, pftpar, wet, zm, zgw)
    % function input:
    % ----------
    % wa      -- soil water content, 3 layers
    ...
    % calculate the moisture constrains for plant and soil in unsaturated zone
    [f_sm1, f_sm_s1] = swc_stress(wa1, soilpar, pEc, pftpar);
    [f_sm2, ~, theta_cs] = swc_stress(wa2, soilpar, pEc, pftpar);
    [f_sm3, ~] = swc_stress(wa3_unsat, soilpar, pEc, pftpar);

    % actual transpiration
    Tr1 = f_sm1 * s_vpd * s_tem * Tr_p1;
    Tr2 = f_sm2 * s_vpd * s_tem * Tr_p2;
    Tr3_u = f_sm3 * s_vpd * s_tem * Tr_p3_u;
    Tr3_g = s_vpd * s_tem * Tr_p3_g;
    Tr3 = Tr3_u + Tr3_g;
    Tr = Tr1 + Tr2 + Tr3;
    ...
    wa3_unsat = max(wa3_unsat, wwp); % > wwp
    wa3_unsat = min(wa3_unsat, theta_sat); % < theta_s
...
% --------------------------------------- %
function [wa, zgw, Tr, Es, uex, theta_cs] = swb_case4(wa, IWS, pEc, pEs, s_tem, s_vpd, ...
    soilpar, pftpar, wet, zm, zgw)
    % function input:
    % ----------
    % wa      -- soil water content, 3 layers
    ...
    wa1 = (wa1 * d1 - f1 - Es - Tr1) / d1;
    wa1 = max(wa1, wwp);
    % wa1 = min(wa1, theta_sat); % < theta_s
    % ff1 = (wa1 - theta_sat) * d1; % extra water from upper layer
    % ff1 = max(ff1, 0);
